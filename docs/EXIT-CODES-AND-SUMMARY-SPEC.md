...
| **3** | VK context (1051) | VK API 1051: метод недоступен для типа профиля/токена. Используется в `upload-one` для fail-fast в пайплайне (прервать batch, не молотить все ID). |

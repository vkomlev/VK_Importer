...
Перед автоматизированным `ship` сначала получить актуальный `release-prep` артефакт для той же ветки/релизного scope.
Если `release-prep` даёт `no-go`, автоматический релиз не выполнять.
...
Применить [общие правила коммитов](/d:/Work/IDE_booster/Docs/ai-booster/git-commit-rules.md).
